...
describe("TimerService with Database", () => {
  ...
  test("listTimers returns paginated results", async () => {
    ...
    insertTestTimer({ uri: "at://test/timer/4", save_count: 0 }); // Should be excluded
    ...
    expect(result.total).toBe(3); // Excludes save_count: 0
    expect(result.timers.length).toBe(2);
    // Should be sorted by save_count DESC
...
describe("TimerService Search", () => {
  ...
  test("searchTimers ranks by save_count and text relevance", async () => {
    ...
    // With saveWeight=1 and textWeight=0, should be sorted by save_count
