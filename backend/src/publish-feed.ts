 ...
 *
 * Usage:
 *   BLUESKY_HANDLE=you.bsky.social BLUESKY_PASSWORD=app-password bun run src/publish-feed.ts
 *
 * This creates an app.bsky.feed.generator record that tells Bluesky
 * where to fetch the feed skeleton from (our server).
 */
...
if (!HANDLE || !PASSWORD) {
  console.error(
    "Usage: BLUESKY_HANDLE=you.bsky.social BLUESKY_PASSWORD=app-password bun run src/publish-feed.ts"
  );
  process.exit(1);
}
...
async function main() {
  ...
  // Build the feed generator record
  const record = {
